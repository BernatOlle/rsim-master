...
//					for (p_node = p_nodes.begin(); p_node != p_nodes.end(); ++p_node) {
						...
						if (Packet_brs_non_p *p_packet = dynamic_cast<Packet_brs_non_p *>(p_nodes[in]->get_in_buffer_front())) {  // ERROR bases operand of '->' has not pointer type 'std::vector<Node*>'
							...
							// If backoff is zero, we transmit first cycle/preamble of packet
								...
								Global_params::Instance()->push_ids_concurrent_tx_nodes(*curr_couple); // why not curr_nodes??
		...
		if (Global_params::Instance()->is_medium_busy()) {
			...
			if (Packet_brs_non_p *p_packet = dynamic_cast<Packet_brs_non_p *>(p_node->get_in_buffer_front())) {
				...
				if (p_packet->get_cycles_left() == 0) {
					...
					Global_params::Instance()->set_medium_idle();
					Global_params::Instance()->update_token_current_node();
					Global_params::Instance()->rotate_tx_weights(
							1); // since we have passed the token to next node, we shift all tx weights one position as well
					if (Global_params::Instance()->is_debugging_on()) {
						std::cout << "TOKEN PASSED" << std::endl;
					}
...
//									   nchannels, 0);
				...
				if (Packet_brs_non_p *p_packet = dynamic_cast<Packet_brs_non_p *>(p_node->get_in_buffer_front())) {
					...
					if (p_packet->get_cycles_left() == 0) {
						...
						p_node->pop_packet_buffer(curr_cycle);
						if (Global_params::Instance()->is_debugging_on()) {
							std::cout << "Packet by " << p_node->get_id() << " has been successfully tx (latency "
									  << lat << ")" << std::endl;
						}
						if (Global_params::Instance()->get_ids_concurrent_tx_nodes_size() == 1) {
							if (Global_params::Instance()->get_unique_ids_concurrent_tx_nodes() == *found_node) {
				...
				if (Global_params::Instance()->get_fuzzy_size() <=
					ceil((Global_params::Instance()->get_thr_pure_token()) * ncores)) {
					Global_params::Instance()->update_token_current_node();
					Global_params::Instance()->rotate_tx_weights(
							1); // since we have passed the token to next node, we shift all tx weights one position as well
					if (Global_params::Instance()->is_debugging_on()) {
						std::cout << "Fuzzy size (" << Global_params::Instance()->get_fuzzy_size()
								  << ") below or equal to threshold (";
						std::cout << ceil((Global_params::Instance()->get_thr_pure_token()) * ncores) << "). PURE TOKEN"
								  << std::endl;
						std::cout << "TOKEN PASSED" << std::endl;
					}
				}
		...
		if (!Global_params::Instance()->is_medium_busy()) {
			...
			for (std::vector<int>::const_iterator curr_node_id = nodes_ready.begin();
				 curr_node_id != nodes_ready.end(); ++curr_node_id) {
				Node *p_node = chip.at(*curr_node_id);
				...
				if (Global_params::Instance()->is_debugging_on()) {
					...
					std::cout
							<< Global_params::Instance()->tx_weights[Global_params::Instance()->mapping_node_ids[*curr_node_id]]
							<< " and picked " << value_picked << std::endl;
				}
			...
			if (fuzzy_nodes_ready.size() == 0) {
				if (Global_params::Instance()->is_debugging_on()) {
					std::cout << "Fuzzy area nodes did not tx (either no packets or tx probability too low)"
							  << std::endl;
				}
...
//						collided_nodes.push_back(std::to_string(p_node->get_id()));
						...
						*//*for (elm in collided_nodes) {
							Node::channel_function("fuzzy_token", "nodes that collided", nchannels, 0)
						}*//*
					}
						// If the cast fails
					else {
				...
				// If fuzzy size is above threshold, we keep txing_token=false to skip 1st period and start next epoch in BRS mode again (pure BRS)
				if (Global_params::Instance()->get_fuzzy_size() >=
					ceil((Global_params::Instance()->get_thr_pure_brs()) * ncores)) {
					if (Global_params::Instance()->is_debugging_on()) {
						std::cout << "Fuzzy size (" << Global_params::Instance()->get_fuzzy_size()
								  << ") above or equal to threshold (";
						std::cout << ceil((Global_params::Instance()->get_thr_pure_brs()) * ncores) << "). PURE BRS"
								  << std::endl;
					}
				}
					// Otherwise (we are in hybrid mode), we turn on token mode and get ready for 1st period of next epoch as usual
				...
				if (Packet_brs_non_p *p_packet = dynamic_cast<Packet_brs_non_p *>(p_node->get_in_buffer_front())) {
					...
					p_packet->decrease_cycles_left();
//					Node::channel_function("fuzzy_token", "1 node transmitting", nchannels, 0)
