...
for i=4:length(files)
    ...
    aa = load(files(i).name);
    H = genhurst(aa(:,1))
    lambda = length(aa)/aa(end,1)/64
    namee = sprintf('trace_npackets-10000_sigma-100_H-%1.1f_injrate-%1.4f.txt',H,lambda);
    movefile(files(i).name,namee);
end
